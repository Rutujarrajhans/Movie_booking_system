...
class MainApp(ttk.Frame):
    ...
    def submit(self):
        with sqlite3.connect('address_book.db') as db:
            c = db.cursor()
        c.execute("INSERT INTO Seats3 VALUES(:no_of_seats)",

                  {
                     
                      'no_of_seats':no_of_seats.get()


                  })
        # db.commit()

        print(no_of_seats,"booked")
    ...
    def query(self):
        with sqlite3.connect('address_book.db') as db:
            c = db.cursor()
        c.execute("SELECT no_of_seats FROM  Seatssys3")
        records = c.fetchall()
    ...
    def make_seats(self):
        ...
        no_of_seats = Entry(self.f1,  bd=10, font=('', 10))
        no_of_seats.grid(row=1, column=1)
        ...
        Button(self.f1, text="query", bd=3, font=('lucida handwriting', 15), padx=5, pady=5, bg="grey",
               command=self.next).grid(sticky=W, row=4, column=1)
